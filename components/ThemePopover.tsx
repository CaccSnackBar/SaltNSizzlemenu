...
const ThemePopover: React.FC<ThemePopoverProps> = ({ themes, currentTheme, onThemeChange, previewTheme, onPreviewTheme }) => {
    ...
    const ThemeSwatch: React.FC<ThemeSwatchProps> = ({ theme }) => (
        ...
        >
          ...
          >
            {/* The visual representation of the theme swatch */}
          ...
          <span className={`text-xs text-center font-medium transition-colors w-16 truncate ${currentTheme.id === theme.id ? 'font-bold' : ''}`} style={{color: displayTheme.colors.textSecondary}}>
            {theme.name}
          </span>
        </button>
      );
      ...
      const ThemeSection: React.FC<ThemeSectionProps> = ({ title, themes }) => (
        <div>
          <h4 className="font-bold text-sm uppercase tracking-wider mb-3 pl-1" style={{ color: displayTheme.colors.textSecondary, fontFamily: displayTheme.fontHeader }}>{title}</h4>
  ...
  return (
    <div 
        className="absolute top-14 right-0 w-[300px] rounded-lg shadow-2xl p-4 z-20 space-y-5 transition-colors duration-200"
        style={{
